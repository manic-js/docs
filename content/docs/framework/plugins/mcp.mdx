...
// manic.config.ts
import { defineConfig } from 'manicjs/config';
declare function mcp(options?: Record<string, unknown>): unknown;
declare function defineTool(name: string, config: Record<string, unknown>): unknown;
const z = {
  string: () => ({ describe: (_s: string) => ({}) }),
  number: () => ({ optional: () => ({ describe: (_s: string) => ({}) }) }),
  object: (_shape: Record<string, unknown>) => ({} as any),
};
declare function fetchUser(id: string): Promise<unknown>;
declare function searchProducts(query: string, limit: number): Promise<unknown>;
...
declare function sendNotification(userId: string, message: string): Promise<void>;
...
declare function searchPosts(keyword: string, limit: number): Promise<unknown[]>;
declare function getPostBySlug(slug: string): Promise<unknown>;
