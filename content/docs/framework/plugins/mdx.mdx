...
declare function mdx(options?: Record<string, unknown>): { name: string };
...
```ts twoslash
// @errors: 2322
declare const metadata: { title: string };
```
...
```ts twoslash
// @errors: 2322
declare const toc: Array<{ depth: number; url: string; title: string }>;
...
- [SEO](/docs/framework/plugins/seo) - Meta tags for content
- [Sitemap](/docs/framework/plugins/sitemap) - Auto-generate sitemap
