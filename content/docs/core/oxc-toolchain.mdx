...
`manic fmt` invokes **`bun x oxfmt -c .oxfmt.json .`** — it **does not forward extra argv**. For **`--check`** / CI dry-runs, call **`oxfmt`** directly ([Lint & format CLI](/docs/cli/lint-fmt)).

```bash
manic fmt           # format workspace via manic wrapper
bun x oxfmt -c .oxfmt.json --check .   # CI-style check (manual)
```
